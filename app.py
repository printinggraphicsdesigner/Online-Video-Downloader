...
def get_video_info():
    """Extract video info and qualities"""
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if direct_url:
                ...
                return jsonify({
                    ...
                    'hint': 'If download fails, try right-click → "Save link as..."'
                })
            else:
                return jsonify({
                    'success': False,
                    'error': 'Direct download not available for this format.',
                    'hint': 'Try a different quality (720p or lower).',
                    'ffmpeg_note': '1080p+ may require FFmpeg.'
                }), 400
